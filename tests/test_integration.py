...
def test_sensor_data_flow(kafka_producer, kafka_consumer):
    ...
    kafka_producer.produce(topic=topic, key=test_data["sensor_id"], value=json.dumps(test_data))
...
def test_ecommerce_events_flow(kafka_producer, kafka_consumer):
    ...
    kafka_producer.produce(topic=topic, key=test_event["user_id"], value=json.dumps(test_event))
    kafka_producer.flush()
...
def test_multiple_messages_ordering(kafka_producer, kafka_consumer):
    ...
    for i in range(num_messages):
        ...
        kafka_producer.produce(topic=topic, key=sensor_id, value=json.dumps(data))
    ...
    assert (
        len(received_values) == num_messages
    ), f"Expected {num_messages} messages, got {len(received_values)}"
    # Verify ordering
    assert received_values == sorted(received_values), "Messages not in order"


@pytest.mark.parametrize(
    "sensor_type,expected_range",
    [("temperature", (-20, 80)), ("humidity", (0, 100)), ("pressure", (900, 1100))],
)
