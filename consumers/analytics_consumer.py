...
from utils.database import AggregatedMetrics, SensorReading
    ...
    def aggregate_window_data(self):
        ...
        try:
            for key, readings in self.window_data.items():
                ...
                df = pd.DataFrame([{"value": r.value, "timestamp": r.timestamp} for r in readings])
        ...
        try:
            while True:
                ...
                if msg is None:
                    ...
                    if (
                        self.message_batch
                        and (datetime.utcnow() - self.last_batch_time).seconds > self.batch_timeout
                    ):
